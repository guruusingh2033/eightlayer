...
@Component({
  selector: 'show-quize',
  templateUrl: './show-quize.component.html',
  styleUrls: ['./show-quize.component.css']
})
export class ShowQuizeComponent implements OnInit {
...
//-----
  ...
  ngOnInit() {
    this.showSpinner = true;
    let userdata = {
      username: "jainpravesh8199@gmail.com",
      entid: localStorage.getItem("Orgnisation_id")
    };
    this.httpClient.post('https://gvb0azqv1e.execute-api.us-east-1.amazonaws.com/dev/clientlogin', userdata)
      .subscribe((res: any) => {
      ...
      .subscribe(response => {
        ...
        this.quizeData.data.filter((x) => {
          if(x.quiz_status === 0 && x.entid === parseInt(this.entId)) {
            ...
            this.showLesson(x.lessons_included[0])
